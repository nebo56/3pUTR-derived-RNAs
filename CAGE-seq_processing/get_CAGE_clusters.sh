...
# 5' read positions
python2 ./scripts/getStart-BED.py ${path}${bam1}.bed ${path}${bam1}.5p.bed
python2 ./scripts/getStart-BED.py ${path}${bam2}.bed ${path}${bam2}.5p.bed

# get sum of 5' reads per position per replicate and for merged samples
python2 ./scripts/BEDsum.py ${path}${bam1}.5p.bed ${path}${bam1}.5p.sum.bed
...
Rscript ./scripts/get_max_cDNA_peak-inside_cluster.R ${path}${bam1}.${bam2}.merged.5p-2min-merged20nt-peaks.bed ${path}${bam1}.${bam2}.merged.5p-2min-merged20nt-max_peaks.bed